...
### Добавлены в KTools/lib (перенесено вручную, подключено в load.xml)
- `AceSerializer-3.0` — `KTools/lib/AceSerializer-3.0/` → `AceSerializer-3.0.xml`
- `LibDeflate` — `KTools/lib/LibDeflate/LibDeflate.lua`
...
```
KToolsAutoloot/
  KToolsAutoloot.toc          ← lib\load.xml убран (AceSerializer+LibDeflate в KTools/lib)
...
### Шаг 1: Подключить библиотеки [new-036] ✅ ВЫПОЛНЕН
**Файлы:** `KTools/lib/load.xml`, `KToolsAutoloot/KToolsAutoloot.toc`
- `AceSerializer-3.0` и `LibDeflate` перенесены в `KTools/lib/` вручную
- `KTools/lib/load.xml` дополнен двумя строками подключения
- `KToolsAutoloot.toc`: мёртвая ссылка `lib\load.xml` удалена
- **Результат:** AceSerializer и LibDeflate доступны через LibStub для всех KTools-аддонов
...
### Шапка (одинакова в обоих режимах)
```
[x] [Профиль v] [Создать] [Удалить v] [Импорт] [Экспорт]  [<другой режим>]
```
Кнопка-свич показывает название **другого** режима. Токены убраны.
...
```
┌─ Импорт профиля ───────────────────────────────────────────────┐
│ [Применить]  [Декодировать]  [Кодировать]                      │
│ ┌────────────────────────────────────────────────────────────┐  │
│ │  <поле ввода — вставить сжатую или plain-строку>           │  │
│ └────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```
- **Применить** — auto-detect формат (DecodeForPrint→Decompress→Deserialize ИЛИ ParseItemsString), применить к профилю
- **Декодировать** — преобразовать содержимое поля из сжатого → plain text (для просмотра)
- **Кодировать** — преобразовать содержимое поля из plain text → сжатый (для копирования)

### IO-диалог экспорта
```
┌─ Экспорт профиля ──────────────────────────────────────────────┐
│ [Закрыть]  [Декодировать]  [Кодировать]                        │
│ ┌────────────────────────────────────────────────────────────┐  │
│ │  <поле вывода — заполняется автоматически при открытии>    │  │
│ └────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```
- Открывается уже заполненным сжатым форматом
- **Закрыть** — закрыть диалог
- **Декодировать** — показать содержимое поля в plain text
- **Кодировать** — вернуть обратно в сжатый формат

---

## Принятые решения

1. **Токены** — убраны из категорий, ключ `tokens` убрать из DEFAULTS и locale.
2. **Кнопка-свич** — текст = **другой** режим (куда переключиться).
3. **Импорт/Экспорт** — две отдельные кнопки в шапке, два разных диалога одинакового размера.
   - Импорт: кнопки `[Применить] [Декодировать] [Кодировать]`
   - Экспорт: кнопки `[Закрыть] [Декодировать] [Кодировать]`
   - Декодировать/Кодировать работают с содержимым поля в обоих диалогах
   - Применить: auto-detect формат (compressed или plain text)
4. **Переключение режима** — перестройка панели **без закрытия окна**.
5. **Колонка ilvl** — заголовок `">= ilvl"`.
